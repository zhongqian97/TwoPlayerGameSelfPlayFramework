...
class WinSumHandlerByEachStepAndSingleAgentInTrain(StrategyHandler):
    def handleRequest(self, store_dict, request_name="WinSum"):
        ...
        for i in range(len(path_name_list)):
            for k in range(len(request_list)):
                xAxis_list = [num for num in range(0, 101, 1)]
                # xAxis_list.append("test")
                data_list = []
                agent_name_list = []
                request_name = request_list[k]
                path = "./" + path_name_list[i] + "/"
                dir_dict = self.getTrainDataDict(path)
                    ...
                    for num in xAxis_list:
                        ...
                        for win_sum_num in request_agent_data_list:
                            if win_sum_num[0] == str(num):
                                if win_sum_num[index] == "True":
                                    win_sum += 1
                                all_sum += 1

                        win_sum_list_by_each_step.append(win_sum / all_sum if all_sum > 0 else -1)
