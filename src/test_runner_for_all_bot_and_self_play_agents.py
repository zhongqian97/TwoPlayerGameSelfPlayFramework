...
def Testing(args):
    ...
    elo_opponents.add_all_test_bots()
    ...
    for epoch in range(x):
        agent_name = None
        for agent_index in range(len(elo_opponents.opp_ai_list)):
            agent = elo_opponents.opp_ai_list[agent_index]
            print(type(elo_opponents.opp_ai_map[agent]))
            if type(elo_opponents.opp_ai_map[agent]) != str:
                del elo_opponents.opp_ai_list[agent_index]
                agent_name = agent
        ...
        for opp in opp_list:
            for p1_flag in [True, False]:
                input_info = {}
                input_info["p1_bot"] = {"name": opp, "agent": elo_opponents.opp_ai_map[opp]}
